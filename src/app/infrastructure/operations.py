from src.config.parameters import db_char_param


def _search_one(table, attributes, identifier):
    var_attributes = _make_attrib_format(attributes)
    return f'SELECT {var_attributes} FROM {table} WHERE {identifier}={db_char_param};'
...
def _insert(table, attributes):
    parameterized = ', '.join([db_char_param] * len(attributes))
    var_attributes = _make_attrib_format(attributes)
    return f'INSERT INTO {table} ({var_attributes}) VALUES ({parameterized});'


def _update(table, attributes, identifier):
    values = f'={db_char_param}, '.join(attributes)
    return f'UPDATE {table} SET {values}={db_char_param} WHERE {identifier}={db_char_param};'


def _delete(table, identifier):
    return f'DELETE FROM {table} WHERE {identifier}={db_char_param};'
